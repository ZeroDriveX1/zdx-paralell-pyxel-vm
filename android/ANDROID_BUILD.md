...
The service is `START_STICKY`, but no task admission occurs unless persisted
settings enable it. The UI supports enable/disable, idle-only, charging, and
...
signals on some versions, so the service rechecks admission every 15 seconds.
