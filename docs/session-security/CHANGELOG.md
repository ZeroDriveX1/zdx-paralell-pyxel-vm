...
## [0.6.0] - 2026-07-30 - Operational Validation

- Added dependency-free runtime metrics for sessions, nodes, commits/failures, recovery, migrations, commit/scheduler latency, lock contention, retries, memory, descriptors, threads, and uptime, with atomic JSON export.
- Added deterministic failure injection for commit phases and registry persistence.
- Added bounded/duration soak, 1,000+ commit stress, resource-leak checks, adverse network scenarios, deterministic fuzzing, and machine-readable benchmarks.
- Locally passed 100 integrated soak cycles, 1,000 atomic commits, 156 tests, and nine benchmarks without descriptor/thread/stale-transaction growth.
- Did not execute the provided 24-hour mode or physical multi-machine/power-loss validation; readiness claims remain bounded to recorded local evidence.

## [0.5.0] - 2026-07-30 - Transactional Persistence

- Added the canonical `StateStore`, atomic byte/secret commits, bounded POSIX locks, fsync, backups, rollback, corruption quarantine, and verified recovery.
- Added versioned/checksummed envelopes with timestamps, compatibility metadata, legacy detection, sequential migrations, module discovery, migration logs, and failure rollback.
- Migrated VM shared state, pixel memory/indexes, coordinator state, karma, identities/key metadata, trusted peers, revocations, session audit metadata, and node registry metadata.
- Preserved legacy JSON and PNG reads; new pixel records carry schema metadata and payload checksums.
- Added interrupted-write, process concurrency, reader coordination, stale-lock, corruption, restart, backward-compatibility, and migration tests.
- Distributed-filesystem and physical power-loss behavior remain deployment validation requirements; PNG encryption remains out of scope.

## [0.4.0] - 2026-07-30 - Authenticated Networking Foundation

- Replaced the live unsigned TCP envelope with one canonical Ed25519-signed format covering routing, session, replay, and payload fields.
- Added pinned key-derived identities, mutual challenges, expiring sessions, strict sequences, nonce defense, timestamp/checksum validation, and authorized versioned key rotation.
- Integrated fail-closed authentication into the TCP coordinator, client, and CLI.
- Hardened registration, reconnects, heartbeat binding, duplicate identities, stale cleanup, and shared-state locking.
- Added production-network tests for authentication, expiry, replay, nonces, duplicate IDs, reconnect, recovery, malformed packets, and TCP behavior.
- TLS, durable trust/revocation, connection-level DoS controls, and physical multi-machine validation remain.
...
## Pass 14.0 — 2026-07-30

- Required TLS 1.3 mutual certificate authentication for coordinator and worker network entrypoints.
- Added hostname validation, optional CRL checking, and reloadable server TLS contexts for new connections.
- Added reconnecting worker service entrypoint and thread-safe scheduler registration/removal integration.
- Added local isolated-process deployment, TLS lifecycle, network-fault, and SIGKILL recovery tests.
- Added RC2 source packaging metadata, Docker/systemd/Windows examples, and deployment/operations guides.
- Recorded that multi-machine, 24-hour, capacity-to-failure, physical power-loss, and service-platform acceptance remain unexecuted.

## Pass 15.0 — 2026-07-30

- Added the `InferenceProvider` abstraction and environment-configured pooled Ollama client, defaulting to `qwen2.5:1.5b` at `http://localhost:11434`.
- Added bounded timeouts, retries with exponential backoff/jitter, structured errors, connectivity checking, and NDJSON streaming compatibility.
- Added scheduler-mediated autonomous missions with memory loading, prompt construction, provider generation, verification, transactional persistence, metrics, and optional PNG mirroring.
- Added disabled-by-default reflection with early success termination and a hard two-cycle ceiling.
- Added deterministic Ollama HTTP integration tests, a mission CLI, and machine-readable stub/live benchmark modes.
- Did not add branching, voting, BAN routing, or distributed inference.

- Installed the Qwen 2.5 1.5B model locally, but did not claim live inference: the host's external Ollama 0.32.3 package lacks `llama-server`; deterministic HTTP integration and graceful HTTP 500 recovery are the verified evidence.
